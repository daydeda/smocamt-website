## RESOLVED 2026-09-19 — both bugs below fixed on branch `fix/checkout-status-display`

**Bug 1** (stale/generic checkout modal): `ScannerService.confirmCheckout` now returns a
distinct `success_checkout` status with the just-awarded points (read back from
`awardIndividualPoints`'s `.returning()`, not the pre-transaction snapshot). Added a
matching `STATUS_CONFIG.success_checkout` entry + `t.scanCheckoutSuccess` i18n string
("Check-out Successful!") in all 4 languages, and included `success_checkout` in the
scan API's `revalidateLeaderboards()` trigger (it was only firing for
`success`/`success_walk_in` before, so a checkout's leaderboard bust was silently
skipped too).

**Bug 2** (stale dashboard "Points" stat): root cause was worse than caching — the
stat was summing `event.pointsAwarded` (the event's HOUSE-winner bonus config field),
not `event.individualPointsAwarded` (what a student actually earns), so it was wrong
for essentially any event, checkout or not. Fixed by having `/api/events` GET expose a
new `pointsCredited` boolean per event (attended AND — for `requireCheckOut` events
only — `checkOutTime` is set), and `DashboardClient`'s `pointsEarned` now sums
`individualPointsAwarded` gated on that flag instead.

Verified end-to-end against the local dev DB by replaying the exact repro below via
direct API calls (browser extension wasn't connected this session) — confirmed via
Postgres that a fresh checkout now returns `status: "success_checkout"` with
`points: 10` (was `"success"` + `0`), and `/api/events` now reports
`pointsCredited: true, individualPointsAwarded: 10` for the checked-out event (and
correctly `false` before checkout completes). `npm test` (289 passed), `npm run lint`
(no new errors — the one pre-existing error is in `battle-flow.test.ts`, untouched by
this change), and `npm run build` all pass.

Not changed: the same "response returns pre-award points" pattern also technically
exists on the plain `confirm`/walk-in success paths (they return `studentWithMedical`
built before the award transaction runs) — untested this session and out of scope for
this fix; flag it if it turns out to matter in practice.
